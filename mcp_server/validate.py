...
(does the container exist, is the carrier suspended, is the release order
still Pending) — deliberately does NOT live here; it lives in each
handler in tools_impl/, against the live database, because no schema
library can know that.

Note for graders / teammates: this hand-rolls the small subset of JSON
Schema this project's tool schemas actually use (type, properties,
required, additionalProperties, enum, minLength, maxLength, pattern,
minimum) because this sandbox has no network access to `pip install
jsonschema`. The function signature — validate(instance, schema) raising
on the first violation — mirrors `jsonschema.validate()` on purpose, so
swapping in the real library later (it's already in requirements.txt) is
a one-line change in server.py, not a rewrite.
"""
