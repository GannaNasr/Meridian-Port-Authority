...
Right now there's exactly one trigger in this server: a session's role
...
affects tool visibility again (e.g. a badge being deactivated mid-session)
it should call this same helper — that's the point of keeping it in one
place instead of inlining send_message calls all over tools_impl/.
"""
