...
This is the core of the "Authorization" and "Notifications" concerns:
  ...
  * Session holds whatever role (if any) the current connection has
    ...
    a client cannot claim a role by just passing a `role` argument.
  * require_role() is called INSIDE every restricted tool handler, not just
    ...
    regardless of what tools/list returned.
"""

from mcp_server.protocol import JSONRPCError, ERR_UNAUTHENTICATED, ERR_UNAUTHORIZED

ROLES = ("dispatcher", "customs_officer", "supervisor")
...
class Session:
    ...
    def login(self, staff_row):
        self.staff_id = staff_row["id"]
        self.name = staff_row["name"]
        self.role = staff_row["role"]
        self.badge_code = staff_row["badge_code"]
    ...
    def require_role(self, *allowed_roles):
        ...
        if not self.authenticated:
            raise JSONRPCError(
                ...
                "tool with a valid badge_code first.",
            )
