...
Ownership note for the README / grader: the three methods below are the
exact points where a Meridian tool crosses into the Elicitation, Sampling,
and Progress-tracking protocol concerns. Person 2 built this plumbing
because it lives inside the server/tool layer; Person 3 is the one who
implements and demos the *client-side* responses (answering
elicitation/create, running sampling/createMessage against their own
model, and rendering the progress notifications) — see agent/.
...
class ToolContext:
    ...
    def sample(self, messages: list, system_prompt: str = None, max_tokens: int = 600) -> dict:
        ...
        over something — used by assess_container_risk. Blocks for the
        response."""
        if not self.session.supports("sampling"):
            raise protocol.JSONRPCError(
                protocol.ERR_CAPABILITY_UNSUPPORTED,
                "This tool needs the client's model to reason over policy "
