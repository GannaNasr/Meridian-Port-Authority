...
class MCPClient:
    def __init__(
        ...
        cwd: working directory to launch the server in (it resolves
             Database/, resources/, prompts/ relative to its own package
        ...
        try:
            if method == "elicitation/create":
                if not self.elicitation_handler:
                    raise RuntimeError("No elicitation handler registered on this client.")
                result = self.elicitation_handler(params.get("message"), params.get("requestedSchema"))
            elif method == "sampling/createMessage":
                if not self.sampling_handler:
                    raise RuntimeError("No sampling handler registered on this client.")
